...
def hand_type(hand):
    ...
    j = card_hand_count['J']
    card_hand_count['J'] = 0
    # print(list(filter(lambda x: x > 0, card_hand_count.values())), " and jokers: ",j)
    ...
    if any((x==3 ) for x in card_hand_count.values()):
        if any((x+j == 2) for x in card_hand_count.values()):
            hand_type = 'Fullhouse'
        else:
            hand_type = 'Three'
    # print(hand_type,'  j: ' ,j)
...
for hands in hands_ordered:
    for hand in hands:
        ...
        print("rank: "+str(hand_rank)+" hand: " + hand+ " bid: " + hand_bid_dict[hand] + " type: " + hand_type(hand))
